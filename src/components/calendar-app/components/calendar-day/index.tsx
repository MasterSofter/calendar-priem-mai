...
export default function Day({setShowEvents, calendarData, filter, isToday, selectedDate, setSelectedDate, date, isActualDate}: DayProps) : JSX.Element {
  ...
  return (
    <div onClick={() => {setSelectedDate(date); setShowEvents(true);}} className={`${isToday ? "bg-current-date" : ""} ${(selectedDate === date) ? "border border-dark border-dark-mode-light" : ""} cursor-pointer hover-effect-up border-2 hover-border rounded-calendar col text-center text-lg-start h-day-cell ${isActualDate ? "" : "text-muted"}`}>
        ...
        <div className="d-none d-lg-flex flex-column">
          {
            categories?.map((item, index) => index < 4
              && <span key={`day-event-${index}`} className="fs-day-event">{item.category}</span>
            )
          }
          {categories && categories?.length > 4 &&
          <span key={`day-event-end`} className="fs-day-event">{`еще +${categories?.length - 4}`}</span>
          }
        </div>
        <div className="d-lg-none text-center">
          <i className={`fa-solid fa-circle text-primary ${(!categories || categories && categories.length == 0 )? "d-none" : ""} ${categories && categories.length < 2 ? "fs-circle-sm" : categories && categories.length < 4 ? "fs-circle-md" : "fs-circle-lg"}`}></i>
        </div>
      </div>
    </div>
  );
